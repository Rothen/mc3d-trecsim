...
TEST(Camera, Transform3Dto2D)
{
    ...
    Tensor d = torch::tensor({-4.122490927804370875e-01, 1.995424107684372617e-01, -6.780183396248970658e-04, 1.457061937740045934e-03, -5.264488616219945710e-02}, TensorRealTypeOption);
    ...
    Point3 PW = torch::tensor({100.0, -200.0, 150.0}, TensorRealTypeOption).reshape({3, 1});

    std::cout << PW << std::endl;

    Point2 p = camera.transform_3d_to_2d(PW);

    Point2 expected_p = torch::tensor({1699.573690735038781, -408.394452051887129}, TensorRealTypeOption).reshape({2, 1});
...
TEST(Camera, Transform2DTo3DOffcenterCamera)
{
    ...
    Point2 p = torch::tensor({1920.0 / 2.0, 1080.0 / 2.0}, TensorRealTypeOption).reshape({2, 1});
