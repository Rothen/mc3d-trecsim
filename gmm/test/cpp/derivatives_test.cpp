...
TEST(Derivatives, WholeModelFitMeters)
{
    Camera<double> camera0 = initCamera0();
    Camera<double> camera1 = initCamera1();
    camera0.t /= 100.0;
    ...
    for (int i = 0; i < 1000; i++)
    {
        RowMatrix<double> theta = (RowMatrix<double>::Random(numBasis, J * 3)) * 50;
...
TEST(Derivatives, ProjectionsMeters)
{
    Camera<double> camera0 = initCamera0();
    camera0.t /= 100.0;
    WorldPoints<double> PWs(3, 1000);

    PWs.row(0) = (Vector<double>::Random(1000).array() + 2) * 15;
    PWs.row(1) = (Vector<double>::Random(1000).array() + 2) * 5;
    PWs.row(2) = (Vector<double>::Random(1000).array() + 2) * 7;
    Vector<double> diffs(1000);
